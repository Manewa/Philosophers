...
/*   Updated: 2025/03/23 20:15:10 by natgomali        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/philosophers.h"

int	meal_philo_done(t_philo *philo, t_infos *infos)
{
	int	meal;
...
int check_philo(t_philo *philo, t_infos *infos)
{
	...
	while (i < infos->nb_philo)
	{
		...
		{
			printf_time("died", actual_time, &philo[i], infos);
			return (1);
		}
		i++;
	}
...
void	return_is_end(t_infos *infos, t_philo *philo)
{		
	int	i;
	int	meal_done;

	while (check_state(infos, philo) == 0)
	{
		meal_done = 0;
